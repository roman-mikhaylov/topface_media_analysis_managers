...
# Человекочитаемые подписи месяцев по month_code
MONTH_LABEL_RU = {
    202211: "ноябрь 2022",
    202212: "декабрь 2022",
    202301: "январь 2023",
...
def rename_month_cols(df: pd.DataFrame) -> pd.DataFrame:
    new_cols = []
    for col in df.columns:
        # Столбцы-месяцы у нас — числа вида 202301; AM оставляем как есть
        if isinstance(col, (int, float)) and col in MONTH_LABEL_RU:
            new_cols.append(MONTH_LABEL_RU[col])
        else:
            new_cols.append(col)
    df.columns = new_cols
    return df
...
pivot_k1 = rename_month_cols(pivot_k1)
...
pivot_k2 = rename_month_cols(pivot_k2)
